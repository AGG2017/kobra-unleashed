      ...
      <div class="w-full md:w-2/3 lg:w-1/2 justify-between flex gap-5">
        <div class="flex-1">
          <a
            ...
            href="https://github.com/AGG2017/kobra-unleashed"
            target="_blank"
            >Kobra Unleashed</a
          >
        </div>
        ...
        <div class="flex-none gap-2">
          <PrinterNaming
            ...
            @set-printer-name="addPrinterName"
          ></PrinterNaming>
        </div>
        <div class="flex-none">
          <select
            v-model="selectedPrinterId"
            @change="selectPrinterId"
            class="select select-bordered w-full max-w-xs"
          >
            <option
              disabled="disabled"
              selected="selected"
              v-if="Object.keys(printers).length == 0"
            >
              No Printer available
            </option>
            <option
              v-for="printerId in Object.keys(printers)"
              :key="printerId"
              :value="printerId"
            >
